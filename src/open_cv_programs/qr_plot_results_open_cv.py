...
def _bin_module_sizes_px(module_sizes: np.ndarray, step_px: int) -> np.ndarray:
    ...
    # биннинг к ближайшему кратному step_px
...
def plot_interactive(results, out_path: Path, dataset_name: str):
    ...
    # --- сглаживание тренда времени (только если точек достаточно) ---
    if len(avg_time) >= 5:
    ...
    # --- (2) столбчатая диаграмма: counts accuracy=1/0 по бинам ---
    fig.add_trace(
        go.Bar(
            x=uniq_m,
            y=n_acc_1,
    ...
    fig.add_trace(
        go.Table(
            ...
            cells=dict(
                ...
                align=["center"] * 5,  # <-- у нас 5 столбцов
                font=dict(size=13),
                height=26,
            ),
        ),
    ...
    fig.update_layout(
        ...
        height=1150,
        width=1600,
        barmode="group",  # можно заменить на "stack", если хотите суммарные столбики
        legend=dict(
            x=1.02,
            y=1,
            bordercolor="gray",
